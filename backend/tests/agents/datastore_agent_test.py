...
async def test_generate_query_success(
    mock_load_prompt, mock_publish_log_info, mock_execute_query, mock_get_semantic_layer
):
    llm = AsyncMock()
    model = "mock_model"
    ...
    llm.chat.assert_called_once_with(
        model, "generate cypher query prompt", "details to create cypher query prompt", return_json=True
    )
